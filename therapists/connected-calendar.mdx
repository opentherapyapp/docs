...
Booked Open Therapy sessions are written to the calendar we created, the moment they are booked. Each event carries the time, the service and a client's first name, and the address when the session is in person. Nothing here can read or write your own calendars.
